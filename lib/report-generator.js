...
class ReportGenerator {
  ...
  async generateHTMLReport(findings, score) {
    ...
    const reportPath = path.join(outputDir, `code-review-report-${this.timestamp}.html`);
...
*Generated by @self-serve/code-review-tool v1.0.0*`;
  ...
  getAnalyzerSummary(analyzer, result) {
    ...
    switch (analyzer) {
      ...
      case 'tests':
        return `${result.coverage || 0}% coverage`;
      case 'complexity':
  ...
  getAnalyzerStats(analyzer, result) {
    ...
    if (result.coverage !== undefined) stats.push(`<span class="stat"><span class="stat-value">${result.coverage}%</span> coverage</span>`);
