...
class AnalyzerBase {
  ...
  async checkSecurity() {
    ...
    if (result.output) {
      try {
        ...
        // Prefer metadata.vulnerabilities counts (npm v8+)
        const meta = auditResult.metadata || auditResult;
        if (meta && meta.vulnerabilities) {
          const high = meta.vulnerabilities.high || 0;
          const critical = meta.vulnerabilities.critical || 0;
          vulnerabilities = high + critical;
        } else if (auditResult.vulnerabilities && typeof auditResult.vulnerabilities === 'object') {
          // Fallback: count entries flagged as high/critical if present
          Object.entries(auditResult.vulnerabilities).forEach(([pkg, vuln]) => {
            const sev = vuln.severity || '';
            if (sev === 'high' || sev === 'critical') {
              vulnerabilities++;
              details.push({ package: pkg, severity: sev, title: vuln.title, url: vuln.url });
            }
          });
        }
      } catch (parseError) {
  ...
  async checkDependencies() {
    ...
    try {
      const auditResult = await this.runCommand('npm audit --audit-level=high --json', 'Checking security vulnerabilities');
      if (auditResult.success) {
        const audit = JSON.parse(auditResult.output);
        const meta = audit.metadata || audit;
        const high = meta?.vulnerabilities?.high || 0;
        const critical = meta?.vulnerabilities?.critical || 0;
        const count = high + critical;
        results.securityVulnerabilities.vulnerabilities = count;
        results.securityVulnerabilities.success = count === 0;
      }
    } catch (error) {
