...
class AnalyzerBase {
   ...
   * Core business logic (controllers, services, utils, config, validation): 80% weight (16 points out of 20)
   * Infrastructure code (routes, middleware): 20% weight (4 points out of 20)
   */
  calculateWeightedCoverage(coverageData) {
    ...
    // Apply weights: 80% core logic, 20% infrastructure (out of 20 total test points)
    ...
    return {
      ...
      breakdown: {
        coreLogic: {
          ...
          weight: 0.8,  // 80% of test score (16 points out of 20)
          points: Math.round(avgCoreLogicCoverage * 0.8 * 20) / 100  // Actual points earned
        },
        infrastructure: {
          coverage: avgInfrastructureCoverage,
          files: infrastructureFiles,
          weight: 0.2,  // 20% of test score (4 points out of 20)
          points: Math.round(avgInfrastructureCoverage * 0.2 * 20) / 100  // Actual points earned
        }
      }
    };
  }
   ...
   * Updated to reflect weighted scoring: 80% for core business logic (16 points), 20% for infrastructure (4 points)
   */
  getTestPriority(filePath) {
    const fileName = path.basename(filePath);
    const content = fs.readFileSync(filePath, 'utf8');
    
    // HIGH PRIORITY (80% weight - 16 points out of 20) - Core business logic
    if (fileName.includes('controller') || fileName.includes('service')) {
      return 'HIGH';
    }
    if (fileName.includes('util') || fileName.includes('helper')) {
    ...
    // LOW PRIORITY (20% weight - 4 points out of 20) - Infrastructure code
    if (fileName.includes('route') || fileName.includes('middleware')) {
      return 'LOW';
    }
